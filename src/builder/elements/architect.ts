...
export class BuilderElementArchitect extends BuilderElement {
    ...
    async setDimension(dimension: Dimension3D): Promise<BuilderElementUpdate[]> {
        return this.setUpdate(await project.architect.connection.request('elements/set-dimension', { element: this.toArchitectData(), dimension: dimension.toJSON() }))
    }

    async init() {
        this.setUpdate(await project.architect.connection.request('elements/init', { element: this.toArchitectData() }))
    }
    ...
    form(): Promise<FormDataInput[]> {
        return project.architect.connection.request('elements/form', { element: this.toArchitectData() })
    }

    editGraph(): Promise<EditGraph> {
        return project.architect.connection.request('elements/edit-graph', { element: this.toArchitectData() })
    }

    async updateForm(updates: FormDataOutput): Promise<BuilderElementUpdate[]> {
        return this.setUpdate(await project.architect.connection.request('elements/update-form', { element: this.toArchitectData(), updates: updates }))
    }

    private setUpdate(update: ArchitectUpdate): BuilderElementUpdate[] {
