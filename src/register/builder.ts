...
export class BuilderRegister extends ObjectRegister<Builder, BuilderRegistry> {
    ...
    fromJson(json: any, parentGeo: GeoRegistry): Builder {
...
export class BuilderRegistry<B extends Builder = any> extends RegistryObject<B> {
    ...
    static readonly MATERIAL = BUILDERS.register(new BuilderRegistry('material', MaterialBuilder.fromJson, (geo) => new MaterialBuilder(Option.random(MaterialBuilder.materialRandomTypeFromGeo(geo).generateDefault()))))
    ...
    static standardFromJson<B extends StandardBuilder>(structure: { geo: GeoRegistry, children: Record<string, { options: Record<string, RandomTypeRegistry>, multiple: boolean }>, options: Record<string, RandomTypeRegistry> }, fromData: (children: Record<string, BuilderChild>, options: Record<string, Option>) => B): (json: any) => B {
    ...
    static oneChildFromJson<B extends OneChildBuilder>(structure: { geo: GeoRegistry, options: Record<string, RandomTypeRegistry> }, fromData: (child: Builder<any>, options: Record<string, Option>) => B): (json: any) => B {
        return (json) => fromData(
